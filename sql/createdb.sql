...
CREATE VIEW usuariosWithAficiones AS SELECT U.*, A.nombre AS aficiones FROM usuarios U JOIN aficiones A ON U.usuarioId = A.usuarioId;

CREATE VIEW usuariosWithCitas AS SELECT U.*, C.nombres AS nombresCita,C.fecha,C.lugar,C.fechaInicio,C.fechaFinalizacion FROM usuarios U JOIN citas C ON U.usuarioId = C.usuarioId;

CREATE VIEW usuariosWithDirecciones AS SELECT D.*, U.nombre FROM usuarios U JOIN direccion D ON U.direccionId = D.direccionId;

CREATE VIEW solicitudesVínculosAmistades AS SELECT V.*, S.usuarioSolicitado, S.usuarioSolicitante, S.fechaSolicitud,
S.fechaAceptacion, S.fechaRevocacion, S.fechaRevocacionAceptacionCumplimentada
FROM VinculosAmistades V JOIN SolicitudesAmistades S ON ((S.usuarioSolicitado OR S.usuarioSolicitante) = V.usuarioId);

CREATE VIEW usersWithPreferencias AS SELECT U.nombre, E.MinimaEdad,E.MaximaEdad,A.aficion,G.prefProv,G.prefMunicipio,
T.MinimaEst ,T.MaximaEst,O.ojos,L.pelo,S.MinimoPeso,S.MaximoPeso
FROM usuarios U NATURAL JOIN preferencias P NATURAL JOIN prefEdad E NATURAL JOIN prefaficiones A NATURAL JOIN preferenciasgeograficas G
 NATURAL JOIN prefEstatura T NATURAL JOIN prefojos O NATURAL JOIN prefpelo L NATURAL JOIN prefpeso S;

CREATE VIEW fotosUsuarios AS SELECT U.*, F.url, F.descripcion, F.nombre AS fotos FROM usuarios U JOIN fotos F ON 
U.usuarioId = F.usuarioId;